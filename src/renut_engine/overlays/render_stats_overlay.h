...
class RenderStatsOverlayDialog : public rex::ui::ImGuiDialog {
public:
    explicit RenderStatsOverlayDialog(rex::ui::ImGuiDrawer* drawer) : rex::ui::ImGuiDialog(drawer) {
        rex::ui::RegisterBind("bind_renut_render_stats_overlay", "F5",
            "Toggle renderer performance overlay", [this] {
                visible_ = !visible_;
                renut::game_activity_stats::SetEnabled(visible_);
            });
    }

    ~RenderStatsOverlayDialog() {
        rex::ui::UnregisterBind("bind_renut_render_stats_overlay");
    }
    ...
    void OnDraw(ImGuiIO&) override {
        ...
        if (!ImGui::Begin("Renderer performance##RenderStatsOverlay", &visible_, flags)) {
            ImGui::End();
            return;
        }
        ...
        if (trace.available) {
            ...
            ImGui::TextDisabled("GPU trace unavailable: set renut_frame_trace = true before launch.");
        }
        ImGui::End();
    }
...
private:
    bool visible_ = false;
    static double Value(const renut::trace_stats::Snapshot& trace, const char* name) {
        const auto found = trace.values.find(name);
        return found == trace.values.end() ? 0.0 : found->second;
    }

    static void Time(const renut::trace_stats::Snapshot& trace, const char* label, const char* value) {
        ImGui::Text("%-28s %7.2f ms", label, Value(trace, value));
    }

    static void Count(const renut::trace_stats::Snapshot& trace, const char* label, const char* value) {
        ImGui::Text("%-28s %7.0f", label, Value(trace, value));
    }

    static void Rate(const renut::trace_stats::Snapshot& trace, const char* label, const char* value) {
        ImGui::Text("%-28s %7.1f ns", label, Value(trace, value));
    }

    static void DrawTrace(const renut::trace_stats::Snapshot& trace) {
        ImGui::Separator();
