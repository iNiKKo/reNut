...
The native renderer (`rexgpu-renut` plugin) builds against a checkout of
...
draw path — lives entirely in this repo's own `src/graphics/renut/`-mirrored files inside the
...
`rexgpu-renut` (the native renderer plugin) and `rexgpu-xenos` (the stock, unmodified-behavior
plugin — useful as a clean A/B reference: set `gpu_plugin = "xenos"` in `renut.toml` to compare
against `gpu_plugin = "renut"`).
